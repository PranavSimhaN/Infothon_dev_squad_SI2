...
export default function FacultyProfile() {
  ...
  useEffect(() => {
    const fetchUserCourses = async () => {
      try {
        const res = await fetch(`/api/faculty/${currentUser._id}/courses`);
      ...
      <div className="flex-wrap flex gap-4 justify-center">
        ...
        <div className="flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md">
          ...
          <div className="flex gap-2 text-sm">
            <span className="text-green-500 flex items-center">{currentUser.program}</span>
      ...
      <div className="flex flex-wrap gap-4 py-3 mx-auto justify-center">
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800">
          <div className="flex justify-between p-3 text-xl font-semibold">
            <h1 className="text-center p-2">Courses Assigned</h1>
          </div>
          <Table hoverable>
            <Table.Head>
            ...
            {userCourses.map((course) => (
              <Table.Body key={course._id} className="divide-y">
                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell className="hidden md:table-cell">{course.courseDescription}</Table.Cell>
                  <Table.Cell>
                    <Link
                      to={`/coursefaculty/${course._id}`}
